...
export default function ProjectFooter() {
  ...
  const homePath = pathname === '/about' && localStorage.getItem('token') ? '/dashboard' : '/'
  return (
    <footer className="border-t border-line dark:border-white/10 bg-white/50 dark:bg-white/[0.02]">
      <div className="max-w-6xl mx-auto px-6 py-5 sm:py-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
        <div>
          <Link to={homePath} className="inline-flex items-center gap-2 font-display font-semibold text-ink dark:text-white">
        ...
        <nav aria-label="Project links" className="flex flex-wrap gap-2 text-sm">
          <Link to={pathname === '/about' ? homePath : '/about'} className="group inline-flex items-center gap-2 rounded-lg px-3 py-2 text-ocean dark:text-teal hover:bg-ocean/5 dark:hover:bg-white/5 transition-colors">
